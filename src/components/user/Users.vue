...
export default {
  data() {
    // 验证邮箱的规则
    var checkEmail = (rule, value, crollback) => {
      // 验证邮箱的正则表达式
      ...
      crollback(new Error('请输入合法的邮箱'))
    }
    // 验证手机号的规则
    var checkMobile = (rule, value, crollback) => {
      // 验证手机号的正则表达式
      const regEMobile = /^(0|86|17951)?(13[0-9]|15[0123456789]|17[678]|18[0-9]|14[57]|199)[0-9]{8}$/
      if (regEMobile.test(value)) {
        // 合法邮箱
        return crollback()
      }
      crollback(new Error('请输入合法的手机号'))
    }
    return {
      // 获取用户列表的参数对象
      queryInfo: {
